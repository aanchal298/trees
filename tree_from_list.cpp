...
struct node * listtotree(struct node *root,struct ll *head)
{
    queue<struct node *> q;
    ...
    while(head)
    {
        ...
        q.pop();
        y->left=newNode(head->data);
        q.push(y->left);
        head=head->next;
        if(head!=NULL)
        {
            y->right=newNode(head->data);
            q.push(y->right);
            head=head->next;
        }
        else y->right=NULL;
...
void push(struct ll** head_ref, int new_data)
{
    ...
    struct ll* new_node = (ll*)malloc(sizeof(ll));
